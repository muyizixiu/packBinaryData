package pack_pro

import (
	"errors"
	"fmt"
)

var (
	version     uint8 = 1
	sub_version uint8 = 1
	headLen     uint8 = 9
)
var decode_arr = [256]uint8{}
var encode_arr = [256]uint8{}

func Pack(v ...interface{}) ([]byte, bool) {
	if data, ok := v[0].([]byte); ok {
		if cmd, ok_ := v[1].(int); ok_ {
			return pack(data, uint16(cmd)) //when out of range,it needs to recover
		}
	}
	return nil, false
}
func pack(v []byte, cmd uint16) ([]byte, bool) {
	v_tmp, checkCode := encode(v)
	v_tmp = WriteInt8(uint8(checkCode%256), v_tmp)
	v_tmp = WriteCmd(cmd, v_tmp)
	v_tmp = WriteInt8(sub_version, v_tmp)
	v_tmp = WriteInt8(version, v_tmp)
	v_tmp = WriteChar("Y", v_tmp)
	v_tmp = WriteChar("B", v_tmp)
	v_tmp = WriteLen(v_tmp)
	return v_tmp, true
}
func checkErr(err error) bool {
	if err != nil {
		fmt.Println("\n", err.Error(), "\n")
		return true
	}
	return false
}
func WriteInt8(i uint8, v []byte) []byte {
	b := make([]byte, 1)
	b[0] = i
	return append(b, v...)
}
func WriteChar(s string, v []byte) []byte {
	b := []byte(s)
	return append(b[:1], v...)
}
func WriteCmd(cmd uint16, v []byte) []byte {
	return WriteInt8(uint8(cmd/256), WriteInt8(uint8(cmd%256), v))
}
func WriteLen(v []byte) []byte {
	length := uint16(len(v) - 7)
	return WriteCmd(length, v)
}
func UnPack(data []byte) ([]interface{}, error) {
	length := uint16(data[0])*256 + uint16(data[1])
	var err error
	var a []interface{} = make([]interface{}, 2)
	test(data, 62)
	test(data[2:(length+9)], 63)
	a[0], a[1], err = unpack(data[2:(length + 9)])
	return a, err
}
func unpack(data []byte) ([]byte, uint16, error) {
	BY := []byte{'B', 'Y', version, sub_version}
	for i, v := range BY {
		if data[i] != v {
			return nil, uint16(0), errors.New("flags are not right!")
		}
	}
	var cmd uint16
	cmd = uint16(data[4])*256 + uint16(data[5])
	checkCode := data[6]
	test(checkCode, 76)
	data_re, checkCode_tmp := decode(data[7:])
	test(checkCode_tmp, 78)
	test(data_re, 81)
	return data_re, cmd, nil
}
func coding(k []byte, arr [256]uint8) ([]byte, uint64) {
	re := make([]byte, len(k))
	var a uint64
	for i, v := range k {
		re[i] = arr[v]
		a += uint64(v)
	}
	return re, a
}
func decode(k []byte) ([]byte, uint64) {
	return coding(k, decode_arr)
}
func encode(k []byte) ([]byte, uint64) {
	return coding(k, encode_arr)
}
func test(v interface{}, line int) {
	fmt.Println("at ", line, ": ", v)
}
